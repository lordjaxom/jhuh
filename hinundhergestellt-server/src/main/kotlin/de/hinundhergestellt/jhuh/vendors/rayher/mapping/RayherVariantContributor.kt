...
class RayherVariantContributor(
    ...
    override fun contribute(variant: UnsavedShopifyProductVariant) =
        dataStore.findByEan(variant.barcode)?.weight?.also { variant.weight = it.toWeight() } != null

    override fun contribute(variant: ShopifyProductVariant) =
        dataStore.findByEan(variant.barcode)?.weight?.let { variant::weight.update(it.toWeight()) } ?: false
}

private fun BigDecimal.toWeight() = Weight(WeightUnit.GRAMS, toDouble())